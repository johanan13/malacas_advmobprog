...
const createItem = async (req, res) => {
  try {
    const { name, description, photoUrl, qtyTotal, qtyAvailable, isActive } = req.body;
    ...
    if (avail > total) {
      return res.status(400).json({ message: "qtyAvailable cannot exceed qtyTotal" });
    }
...
const updateItem = async (req, res) => {
  try {
    ...
    // Validate qty relationship if present
    if (payload.qtyTotal !== undefined || payload.qtyAvailable !== undefined) {
      ...
      const avail = payload.qtyAvailable !== undefined ? Number(payload.qtyAvailable) : current.qtyAvailable;
...
const getItemByName = async (req, res) => {
  try {
    const item = await Item.findOne({ name: req.params.name, isActive: true });
    if (!item) {
